<template>
  <div id="chatManagePage">
    ...
    <a-form layout="inline" :model="searchParams" @finish="doSearch">
      ...
      <a-form-item label="创建并发送">
        <a-input v-model:value="tempPrompt" placeholder="输入用户需求（initPrompt）" style="width: 320px" />
      </a-form-item>
      <a-form-item>
        <a-button type="primary" :loading="creating" @click="createAndSend">新建并发送</a-button>
      </a-form-item>
    </a-form>
    <a-divider />
...
// 创建应用并发送 initPrompt，随后跳转到聊天页
const createAndSend = async () => {
  if (!tempPrompt.value.trim()) {
  ...
  try {
    creating.value = true
    // 先创建应用
    const res = await (await import('@/api/appController')).addApp({ initPrompt: tempPrompt.value.trim() })
    ...
    const prompt = encodeURIComponent(tempPrompt.value.trim())
    // 跳转到聊天页，交由 AppChatPage 触发一次 SSE 发送
    await router.push(`/app/chat/${appId}?gen=true&prompt=${prompt}`)
  } catch (e) {
    message.error('创建并发送失败')
  } finally {
    creating.value = false
  }
}
